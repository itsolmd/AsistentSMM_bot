...
const process = {};
process.env = {};
process.env.GOOGLE_MAPS_KEY = "AIzaSyBCdx4VD3HFCNVe_ahHTLpYGCjtxRrYHI8";

///////////////////////////////////
// global_url =
//   "https://immobiliare.md/place/townhouse-cu-3-niveluri-or-durlesti-3/";

global_url =
  "https://immobiliare.md/place/penthouse-cu-4-camere-sect-telecentru/";
//type => extract info
const scrap_immobiliare = async (ctx, url) => {
  ...
  // //linkul la google maps, lucreaza, sta comentat sa nu faca prea multe requesturi
  ...
  // //pina aici se formeaza linkul la harta

  // const geolocation = await axios.get(geocodingUrl);
  // objectToSend.geolocation = geolocation.data.results[0].geometry.location;

  let images = [];
  let hero_image = cheerio
    .load($(".row.row-10.wrapper").html())("a")
    .attr("href");
  images.push(hero_image);

  let small_image = cheerio
    .load($(".row.row-10.wrapper").html())(".row.row-10")
    .find("a")
    .each((index, img) => {
      let imgSrc = $(img).attr("href");
      if (imgSrc) {
        images.push(imgSrc.startsWith("//") ? "https:" + imgSrc : imgSrc);
      }
    });

  let small_hidden_image = cheerio
    .load($(".row.row-10.wrapper").html())(".row.row-10")
    .find("a")
    .each((index, img) => {
      let imgSrc = $(img).attr("href");
  ...
  const region = $(".property-title").first().text();
